...
**Android app icon, fixed properly** — the icon files now live directly in `capacitor-project/android-icons/`, mirroring the real Android resource folder structure (`mipmap-mdpi`, `mipmap-hdpi`, etc., plus the adaptive icon XML and background colour). The build workflow copies these straight into `android/app/src/main/res/` after the Android platform is created — no dependency on an asset-generation tool that could silently fail. This replaces the previous `capacitor-project/assets/` + `@capacitor/assets` approach entirely.

**Microphone, properly diagnosed and fixed** — the old implementation only used the Web Speech API, which generally doesn't exist inside Android's WebView at all, so the button likely never appeared in the compiled APK. It now tries the native `@capacitor-community/speech-recognition` plugin first (added as a dependency — this is a real, actively maintained plugin that talks to Android's native speech recognizer), with the Web Speech API kept as a fallback for testing the plain web/PWA version in a browser. It requests microphone permission properly, shows a visible listening state, and surfaces specific error text (permission denied, no speech detected, unsupported) instead of failing silently — you always see why it didn't work, and can fall back to typing.

I still can't personally test either fix inside a real compiled APK on a device — that's the genuine limit of this environment. Both are built the standard, correct way for their respective problems, so the next build/install is the real test.
